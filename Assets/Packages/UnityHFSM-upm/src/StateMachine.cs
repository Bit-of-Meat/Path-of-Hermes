...
namespace FSM
{
		...
		IStateMachine<TStateId>
	{
		...
		public StateMachine<TOwnId, TStateId, TEvent> AddState(TStateId name, StateBase<TStateId> state)
		{
			state.fsm = this;
			state.name = name;
			state.Init();
		...
		public StateMachine<TOwnId, TStateId, TEvent> AddTransition(TransitionBase<TStateId> transition)
		{
		...
		/// </summary>
		public StateMachine<TOwnId, TStateId, TEvent> AddTransition(
			TStateId from,
			TStateId to,
			Func<Transition<TStateId>, bool> condition = null,
			bool forceInstantly = false)
		{
			return AddTransition(CreateOptimizedTransition(from, to, condition, forceInstantly));
		}
